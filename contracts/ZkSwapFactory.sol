...
import './ZkSwapPair.sol';
import '../../interfaces/protocol/core/IZkSwapFactory.sol';

contract ZkSwapFactory is IZkSwapFactory {
    ...
    function createPair(address tokenA, address tokenB) external override returns (address pair) {
        ...
        pair = address(new ZkSwapPair(token0, token1));
    ...
    function setSwapFeeOverride(address _pair, uint16 _swapFeeOverride) external override onlyFeeToSetter {
        ZkSwapPair(_pair).setSwapFeeOverride(_swapFeeOverride);
    }
}
