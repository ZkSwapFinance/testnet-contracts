...
contract ZkSwapPair is IZkSwapPair, ERC20WithPermit, ReentrancyGuard {
    ...
    constructor(address _token0, address _token1) {
        ...
        if (success0 && success1) {
            _initializeMetadata(
                string(abi.encodePacked("ZkSwap ", symbol0, "/", symbol1, " LP Token")),
                string(abi.encodePacked(symbol0, "/", symbol1, " ZLP"))
            );
        } else {
            _initializeMetadata(
                "ZkSwap LP Token",
    ...
    function getSwapFee() public view override returns (uint16) {
        ...
        return _swapFeeOverride == SWAP_FEE_INHERIT ? IZkSwapFactory(factory).swapFee() : _swapFeeOverride;
    }
    ...
    function _tryMintProtocolFee(uint112 _reserve0, uint112 _reserve1) private {
        uint _kLast = kLast;
        if (_kLast != 0) {
            IZkSwapFactory _factory = IZkSwapFactory(factory);
