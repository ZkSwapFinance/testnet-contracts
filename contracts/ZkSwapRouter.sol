...
import './ZkSwapRouterInternal.sol';

import '../../interfaces/IWETH.sol';
import '../../interfaces/protocol/IZkSwapRouter.sol';
import '../../interfaces/protocol/core/IZkSwapFactory.sol';

import '../../libraries/protocol/ZkSwapLibrary.sol';
import '../../libraries/token/ERC20/utils/TransferHelper.sol';

contract ZkSwapRouter is IZkSwapRouter, ZkSwapRouterInternal {
    ...
    function _addLiquidity(
        ...
        pair = ZkSwapLibrary.pairFor(_factory, tokenA, tokenB);
        if (pair == address(0)) {
            // create the pair if it doesn't exist yet
            pair = IZkSwapFactory(_factory).createPair(tokenA, tokenB);
    ...
    function addLiquidity(
        ...
        liquidity = IZkSwapPair(pair).mint(to);
    ...
    function _permit(
        ...
        IZkSwapPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
        return pair;
    }
    ...
    function swapExactTokensForTokens(
        ...
        amounts = ZkSwapLibrary.getAmountsOutUnchecked(factory, amountIn, path); // will fail below if path is invalid
        // make sure the final output amount not smaller than the minimum
        require(amounts[amounts.length - 1] >= amountOutMin, 'INSUFFICIENT_OUTPUT_AMOUNT');

        address tokenIn = path[0];
        address initialPair = ZkSwapLibrary.pairFor(factory, tokenIn, path[1]);
    ...
    function swapExactETHForTokens(
        ...
        amounts = ZkSwapLibrary.getAmountsOutUnchecked(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function swapExactTokensForETH(
        ...
        amounts = ZkSwapLibrary.getAmountsOutUnchecked(factory, amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function swapTokensForExactTokens(
        ...
        amounts = ZkSwapLibrary.getAmountsInUnchecked(factory, amountOut, path); // will fail below if path is invalid
    ...
    function swapETHForExactTokens(
        ...
        address tokenIn = path[0];
        require(tokenIn == WETH, 'INVALID_PATH');
        amounts = ZkSwapLibrary.getAmountsInUnchecked(factory, amountOut, path);
    ...
    function _swapSupportingFeeOnTransferTokens(address initialPair, address[] calldata path, address _to) internal virtual {
        for (uint i; i < path.length - 1; ) {
            ...
            IZkSwapPair pair = IZkSwapPair(i == 0 ? initialPair : ZkSwapLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            ...
            { // scope to avoid stack too deep errors
                ...
                amountOutput = ZkSwapLibrary.getAmountOut(amountInput, reserveIn, reserveOut, swapFee);
            }

            address to = i < path.length - 2 ? ZkSwapLibrary.pairFor(factory, output, path[i + 2]) : _to;
