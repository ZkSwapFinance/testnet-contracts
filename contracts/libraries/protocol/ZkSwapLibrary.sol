...
library ZkSwapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal view returns (address pair) {
        return IZkSwapFactory(factory).getPair(tokenA, tokenB);
    }
    ...
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint112 reserveA, uint112 reserveB, uint16 swapFee) {
        ...
        if (pair != address(0)) {
            (uint112 reserve0, uint112 reserve1, uint16 _swapFee) = IZkSwapPair(pair).getReservesAndParameters();
